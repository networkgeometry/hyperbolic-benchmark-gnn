...
// Editted: In this version of code we save the coordinates of the nodes with zero degree in the unipartite network becuase we will need 
//these coordinate for computation of OS (Predictabiliy bound) as it focuses on an ensemble not indivisual realizations 
...
/*
   ...
   int** Edgelist_s = (int**) malloc(Edge_num * sizeof(int*));
    ...
    for(int i=0; i<Edge_num; i++)
    {
     ...
     printf("\nNs_obs - NGZ is %d", Ns_obs - NGZ);
     int * Z_nodes= (int *) malloc ((Ns_obs - NGZ) * sizeof(int));
     int ix=0;
     for(int m=0; m<Ns_obs ; m++)
     {
     	int suc=0;
     	for (int n=0; n<NGZ ; n++)
     	{	
     ...
     for(int bb=0; bb<ix; bb++)
     {
     	fprintf(fp2, "%d\t%f\t%f\t%d\n", bb+NGZ, kappa_s[Z_nodes[bb]], Theta_s[Z_nodes[bb]], -1);
     } 
