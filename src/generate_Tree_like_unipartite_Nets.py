    ...
    for New_Beta_s_val in New_Beta_s:
        ...
        for i in range(ntimes):
            for (Beta_s_val, gamma_s_val, Ns_obs_val, kmean_s_val, gamma_n_val, kmean_n_val, gamma_f_val, N_f_val, Beta_bi_val, nu_val, alpha_val, N_labels_val) in itertools.product(*all_params):
                ...
                command = f'./main_robert3 {Beta_s_val} {gamma_s_val} {Ns_obs_val} {kmean_s_val} {gamma_n_val} {kmean_n_val} {gamma_f_val} {N_f_val} {Beta_bi_val} {nu_val} {alpha_val} {N_labels_val} {output_folder} > log.txt'
                print(command)
                os.system(command)
                time.sleep(1)
                ...
                log_file= new_output_folder + "/log.txt"
                ...
                command = f'./main_Tree  {Unipartite_Net} {Coord_file} {mu} {Beta_s_val} {New_Beta_s_val} > log_GR.txt'
                print(command)
                os.system(command)               
                os.system(f'mv log_GR.txt {new_output_folder}/log_GR.txt')
